...
export class UserComponent implements OnInit {
  ...
  async verUsuario(user: User): Promise<void> {
    ...
    if (user._id) {
      this.userService.getOne(user._id).subscribe({
        next: (userCompleto: User) => {
          this.usuarioVer = userCompleto;
          
          // Obtener la empresa y las categorías de la empresa
          if (userCompleto.empresaId && typeof userCompleto.empresaId === 'string') {
            this.empresaService.getOne(userCompleto.empresaId).subscribe({
              next: (empresa) => {
                // Obtener las categorías de la empresa
                this.categoriaService.getAll(1, 100, '').subscribe({
                  next: (categoriasResponse) => {
                    console.log('Todas las categorías:', categoriasResponse.items);
                    console.log('ID de empresa del usuario:', userCompleto.empresaId);
                    
                    const categoriasEmpresa = categoriasResponse.items?.filter(cat => {
                      console.log('Categoría:', cat.nombre, 'empresaId:', cat.empresaId);
                      // Comparar como strings para evitar problemas de tipo
                      return String(cat.empresaId) === String(userCompleto.empresaId);
                    }) || [];
                    
                    console.log('Categorías filtradas:', categoriasEmpresa);
                    
                    // Agregar la empresa y categorías al usuario
                    this.usuarioVer = {
                      ...userCompleto,
                      empresaId: {
                        ...empresa,
        ...
        error: () => {
          // Si falla, usar el usuario que ya tenemos
          this.usuarioVer = user;
          this.mostrarModalUsuario();
        }
      });
    } else {
      // Si no hay ID, usar el usuario que ya tenemos
      this.usuarioVer = user;
      this.mostrarModalUsuario();
    }
  }

  private async mostrarModalUsuario(): Promise<void> {
    if (this.usuarioVer) {
      this.testsCompletadosCount = await this.getTestsCompletadosCountAsync(this.usuarioVer);
    }
  ...
  getCategoriasEmpresa(user: User): string {
    if (user.empresaId && typeof user.empresaId === 'object' && (user.empresaId as any).categorias) {
      const categoriasEmpresa = (user.empresaId as any).categorias;
      if (categoriasEmpresa && categoriasEmpresa.length > 0) {
        const categorias = categoriasEmpresa
          .filter((cat: any) => cat && cat.nombre)
          .map((cat: any) => cat.nombre)
          .join(', ');
        
        return categorias || 'Sin categorías asignadas';
      }
    }
